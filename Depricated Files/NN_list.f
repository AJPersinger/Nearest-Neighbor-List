...
cc
        ...
        character (len=100) outfile_ovito
        character*132 fnameroot,file1,fileR,fphi,fvort,fvel,f2strain
      character*132 fcase,fgeo,fcsym,fdgr,fvec,f1stress,f1strain
      character*132 frot,fucg,fcurl,feng,fcna,fgradphi,ftype,f2stress
        character*132 fstrain, fstress,fcna_data,fcna_grain_data
        character*132 fcna_metric_data,fphi_dist,fw_dist,fsvec_dist
        character*132 fgrain_data,fs1_dist,fst1_dist,fdisl_data
        character*132 fcna1_eng_bin,fcna2_eng_bin,fcna5_eng_bin
      character*132 fs1_dist_1,fs1_dist_5,fs1_dist_6,fs1_dist_7
      integer nbegin,nend,nincr,nsteps,ilen
      character*10 nub,str
      character*4 nuc
      character*3 nuc3,forceflag
      character*2 nuc2
      character*1 nuc1
        real*8 Tcsym,Tcna,Ts(6),Teng,dimflag,perlb(3),perub(3),perlen(3)
        real*8,allocatable::csymi(:),engi(:),cnai(:)
        real*8 v1,v2,v3,vi1,vi2,vi3,stmp(3),rcut,rcut2,f0(4)
        real*8,allocatable::si(:,:),h1stress(:),vort(:,:),h2stress(:)
      integer natoms,sflag,ikval,ntime,iforceflag,maxtype,slipct(2)
c slipct is used to determine the # of slipped atoms, where the first
c spot is for partial slip count and the second for full slip

        parameter (neimax=50,nregmax=2)
        real*8,allocatable::r(:,:),Rf(:,:),v(:,:),eng(:),s(:,:),W(:,:,:)
        real*8 reflen(3),refub(3),reflb(3)

        real*8,allocatable::disnbr(:,:,:)
        integer,allocatable::inbr(:,:),numnbr(:)
        real*8 alat
...
c********************************************************************
	...
	fileR = 'out.Pt.0'
c read in reference coordinate file
      open(12,file=fileR,status='old')
      ...
      read(12,*)
      do j = 1,3
         read(12,*) reflb(j),refub(j)
         reflen(j) = refub(j) - reflb(j)
      enddo
      read(12,*)


        allocate(r(3,natoms),Rf(3,natoms))
          allocate(disnbr(6,neimax,natoms),numnbr(natoms))
          allocate(inbr(neimax,natoms))



	do i=1,natoms
	read(12,*) itmptag,itmptype,x,y,z
	r(1,itmptag) = x
	r(2,itmptag) = y
	r(3,itmptag) = z
	 Rf(1,itmptag)= x
